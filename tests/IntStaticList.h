...
typedef int Int_STATIC_LIST_TYPE;
...
typedef struct StaticLists {
	int num_elements;
	size_t size;
	Int_STATIC_LIST_TYPE *array;
} IntStaticList;

void _checkIndex(IntStaticList *list, int index) {
...
void _checkAlloc(IntStaticList *list) {
	if (list->array == NULL) {
...
void _tryRealloc(IntStaticList *list, int num_elements) {
	list->num_elements += num_elements;
	list->array = realloc(list->array, list->num_elements * list->size);
	_checkAlloc(list);
}

void initList(IntStaticList *list, int num_elements) {
	list->num_elements = num_elements;
	list->size = sizeof(Int_STATIC_LIST_TYPE);
	list->array = calloc(num_elements, list->size);

	_checkAlloc(list);
}

void initListAtValue(IntStaticList *list, int num_elements, Int_STATIC_LIST_TYPE value) {
...
void initListAtValues(IntStaticList *list, int num_elements, Int_STATIC_LIST_TYPE values[]) {
	initList(list, num_elements);
	for (register int i = 0; i < num_elements; i++) {
...
void insertValueAtIndex(IntStaticList *list, Int_STATIC_LIST_TYPE element, int index) {
...
Int_STATIC_LIST_TYPE getValueAtIndex(IntStaticList *list, int index) {
	_checkIndex(list, index);
	Int_STATIC_LIST_TYPE element = *(list->array + index);
	return element;
}

Int_STATIC_LIST_TYPE removeValueAtIndex(IntStaticList *list, int index) {
	_checkIndex(list, index);
	Int_STATIC_LIST_TYPE removed = *(list->array + index);
...
void addValueAtEnd(IntStaticList *list, Int_STATIC_LIST_TYPE element) {
	_tryRealloc(list, 1);
	*(list->array + list->num_elements - 1) = element; // num_elements already increased
}

void addValuesAtEnd(IntStaticList *list, Int_STATIC_LIST_TYPE elements[], int num_elements) {
...
void printList(IntStaticList *list) {
	...
	Int_STATIC_LIST_TYPE elem = getValueAtIndex(list, 0);
